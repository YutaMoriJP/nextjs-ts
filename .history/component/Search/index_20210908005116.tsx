...
const Search = ({ data }: BlogProps): JSX.Element => {
  ...
  const handleSubmit = async (
    ...
    //dynamically import fuse module
    ...
    //set result, not really needed right now, but maybe for a different feature
    setResult({ loading: false, result, notFound: result.length === 0 });
    //this cleans up input field
    setHasSubmitted(prev => !prev);
    //if user is found, then push user to that page
    if (id) {
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className={style.form}>
        ...
        {notFound && (
          <p id="resultError" className={style.alert}>
            Not found...
          </p>
        )}
        {loading && <p>LOADING...</p>}
