...
const Home = (): JSX.Element => {
  ...
  const previousUser = usePrevious(user, true);

  const [userData, setData] = useState<IndexData>({
    data: null,
    msg: "",
    loggedIn: false,
    loading: false,
  });
  const { data, msg, loggedIn, loading } = userDaa;
  const [renderData, setRenderData] = useState(null);
  useLocalStorage(storageKey, data, setRenderData);

  useEffect((): (() => void) => {
    if (user !== null && user === previousUser) return;
